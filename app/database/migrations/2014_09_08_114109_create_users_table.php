 ...
 */
    public function up()
    {
    	if (!Schema::connection('akazohoUsers')->hasTable('users')) {
	        Schema::connection('akazohoUsers')->create('users', function($t) {
	            $t->bigIncrements('userId')->unsigned();
	            $t->string('email', 255)->unique();
	            $t->string('password', 128);
	            $t->smallInteger('userType')->unsigned()->default(1)->comment('1->user, 2->admin');
 ...
 */
    ...
    {
        Schema::connection('akazohoUsers')->drop('users');
    }
