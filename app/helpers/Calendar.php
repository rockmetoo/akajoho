...
class Calendar {
	...
	private $monthIco = 'Month';
	private $monthClass = '';
	
	private $weekIco = 'Week';
	private $weekClass = '';
	...
	public function setMonthIcon($html) {
		$this->monthIco = $html;
		return $this;
	}
	
	public function setMonthClass($class) {
		$this->monthClass = $class;
		return $this;
	}
	
	public function setWeekIcon($html) {
		$this->weekIco = $html;
		return $this;
	}
	
	public function setWeekClass($class) {
		$this->weekClass = $class;
		return $this;
	}
	
	public function setDayIcon($html) {
		$this->dayIco = $html;
		return $this;
	}
	
	public function setDayClass($class) {
		$this->dayClass = $class;
		return $this;
	}
	...
	private function buildHeader() {
		...
		if ($this->nav) {
			...
			$h .= "</th>";
			$h .= "<th class='text-center' colspan='$cs'>";
			$h .= $month_name;
			$h .= "</th>";
			$h .= "<th>";
			$h .= "<a class='" . $this->monthClass . "' href='" . $this->monthLink() . "'>" . $this->monthIco . "</a>&nbsp;";
			$h .= "<a class='" . $this->weekClass . "' href='" . $this->weekLink() . "'>" . $this->weekIco . "</a>&nbsp;";
			$h .= "<a class='" . $this->dayClass . "' href='" . $this->dayLink() . "'>" . $this->dayIco . "</a>";
			$h .= "</th>";
		} else {
			$h .= "<th colspan='7'>";
			$h .= $month_name;
		...
		for ($i = $startingDay == 7 ? 1 : 0; $i < 9; $i++) {
			for ($j = 0; $j <= 6; $j++) {
				...
				if ($day <= $monthLength && ($i > 0 || $j >= $startingDay)) {
					$h .= $this->dayWrap[0];
					$h .= "<a href='/mycalendar/add/event/" . strtotime($curr_date) . "'>" . $day . "</a>";
					$h .= $this->dayWrap[1];
	...
	private function monthLink() {
		...
		$url = $this->basePath . '?cv=&cdate=' . $time;
		return $url;
	}
	
	private function weekLink() {
		$y = $this->year;
		$d = intval($this->day);
		$d = $d < 10 ? '0' . $d : $d;
		$m = intval($this->month);
		...
		$url = $this->basePath . '?cv=week&cdate=' . $time;
		return $url;
	}
	
	private function dayLink() {
		$y = $this->year;
		$d = intval($this->day);
		$d = $d < 10 ? '0' . $d : $d;
		$m = intval($this->month);
		...
		$url = $this->basePath . '?cv=day&cdate=' . $time;
		return $url;
	}
